...
var _domutil = require('./src/domutil');

var domutil = _interopRequireWildcard(_domutil);

var _domevent = require('./src/domevent');
...
},{"./src/domevent":299,"./src/domutil":300,"babel-polyfill":2}],2:[function(require,module,exports){
(function (global){
/* eslint max-len: 0 */
...
},{"babel-regenerator-runtime":297,"core-js/fn/regexp/escape":3,"core-js/shim":296}],3:[function(require,module,exports){
...
function on(element, types, handler, context) {
    if (util.isString(types)) {
        types.split(/\s+/g).forEach(function (type) {
    ...
    util.forEach(types, function (func, type) {
...
function addClass(element) {
    for (var _len = arguments.length, cssClass = Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) {
        cssClass[_key - 1] = arguments[_key];
    }
    ...
    if (element.classList) {
        var _ret = function () {
            ...
            cssClass.forEach(function (name) {
                classList.add(name);
            });
    ...
    if (origin) {
        cssClass = [].concat(_toConsumableArray(origin.split(/\s+/)), _toConsumableArray(cssClass));
    }

    var newClass = [].concat(_toConsumableArray(new Set(cssClass))).join(' ');
...
function removeClass(element) {
    for (var _len2 = arguments.length, cssClass = Array(_len2 > 1 ? _len2 - 1 : 0), _key2 = 1; _key2 < _len2; _key2++) {
        cssClass[_key2 - 1] = arguments[_key2];
    }
    ...
    var origin = getClass(element).split(/\s+/);
    var classes = origin.filter(function (name) {
...
function setBound(element) {
    ...
    util.forEach(args, function (value, prop) {
...
var matchSelector = elProto.matches || elProto.webkitMatchesSelector || elProto.mozMatchesSelector || elProto.msMatchesSelector || function (selector) {
    ...
    var match = findAll(doc, selector).find(function (el) {
        return _this === el;
    });
...
function findAll(element, selector) {
    if (util.isString(element)) {
        return [].concat(_toConsumableArray(document.querySelectorAll(element)));
    }
